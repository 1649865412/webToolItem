...
// 获取用户信息接口url
    ...
    String  responsePath = "/customer/anonymousRegister.html?useremail="+mediaUserID+"&username="+username;
    String path = webPath.concat(responsePath);
	//response.sendRedirect(webPath＋"/customer/anonymousRegister.html?useremail="+mediaUserID+"&username="+username);
%>
